...
# For Vercel, the API is on a relative path.
if "VERCEL_URL" in os.environ:
    # In Vercel, the API is at a relative path. We construct the full URL.
    API_URL = f"https://{os.environ['VERCEL_URL']}/api/decide/churn/"
else:
    # For local development, the API is at a different URL.
    # The FastAPI app's endpoint is at the root, not under /api.
    API_URL = "http://localhost:8000/decide/churn/"
...
if submitted:
    ...
    try:
        ...
        st.info("Please ensure the FastAPI server is running. You can start it with: `uvicorn api.main:app --host 0.0.0.0 --port 8000`")